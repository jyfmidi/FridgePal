...
| `OQ-04` | Seed-library breadth and initial bilingual translations. | **Resolved:** Seed the approved bilingual 70-food fresh-food catalog through a versioned, non-destructive database startup operation. Keep `rice` and `pasta` only as inactive compatibility definitions for the existing demo inventory; they are not Add Food presets. |
